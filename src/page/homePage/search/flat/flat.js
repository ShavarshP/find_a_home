...
const Flat = (props) => {
  ...
  const [status, setStatus] = useState({ rent: false, sale: false });
    ...
    <div className={props.state.filterClassName[0]}>
      <div className="flex-container2" style={{ display: "flex" }}>
        ...
        <input
          name="acceptTerms"
          type="checkbox"
          onChange={() => setStatus({ ...status, rent: !status.rent })}
          checked={status.rent}
        />
        <div></div>
      </div>
      <form onChange={handleSubmit(search)} onSubmit={handleSubmit(onSubmit)}>
